...
function graviditetsmails_form()
{
    ...
    $month = array_search($_POST['month'],$months) ?? date('m');
    ...
    $LastMenstruationDate = $date->format('Y-m-d');
    ...
    $request = [
        ...
        'Mode' => 0,
        'DeliveryDate' => date('Y-m-d', strtotime($LastMenstruationDate.' +280 days')) , //strtotime('+'.$delivery.' days', strtotime($date))),
...
function babymails_form()
{
    ...
    $response = curl_exec($curl);
//var_dump($response); var_dump($request); die();
    curl_close($curl);
    wp_redirect( '/termin.htm',302 );
